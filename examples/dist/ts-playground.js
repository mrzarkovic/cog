...
function assignDependents2(elementId, expressions, attributes) {
  expressions.map(function (expression) {
    expression.dependencies.forEach(function (dependency) {
...
function registerCustomElement(template, state, reactiveNodes) {
  return function () {
    ...
    for (var i = 0; i < elements.length; i++) {
      ...
      var _elementId = reactiveNodes.id();
      var _template = element.outerHTML;
      state.registerTemplateState(templateName, _elementId);
      var templateState = {};
      if (state.templates && state.templates[templateName]) {
        templateState = state.templates[templateName].customElements[_elementId];
      }
      var localState = attributesToState(attributes, Object.assign({}, parentState, templateState));
      var newElement = registerReactiveNode(_elementId, reactiveNodes, element, _template, localState, attributes, parentId, templateName);
      if (newElement.nodeType !== Node.TEXT_NODE) {
        addAllEventListeners(newElement, localState);
      }
      newElement.cogAnchorId = _elementId;
    }
