...
function removeTagsAndAttributeNames(htmlString) {
  return htmlString.replace(tagRegex, "").replace(attrRegex, "$1").replace(specialCharRegex, " ").replace(spaceRegex, " ");
}
...
function registerReactiveNode(elementId, reactiveNodes, originalElement, template, state) {
  ...
  var templateForUpdateCheck = refinedTemplate + " " + attributesRecursive.map(function (a) {
    return a.value;
  }).join(" ");
